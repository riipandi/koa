...
fn test_monomorphized_function() -> Result<()> {
    let ir = lower_to_ir("fn identity<T>(x: T): T { return x; } fn main(): void { identity<i32>(42); identity<f64>(3.14); }")?;
...
fn test_monomorphized_struct() -> Result<()> {
    let ir = lower_to_ir("struct Box<T> { value: T; } fn main(): void { let b1: Box<i32> = Box<i32> { value: 42 }; let b2: Box<f64> = Box<f64> { value: 3.14 }; }")?;
